  ...
  Widget build(BuildContext context) {
    return BlocBuilder<DatabaseBloc, DatabaseState>(
      builder: (context, state) {
        ...
        return state.mapTransactions.entries.isEmpty
            ? SizedBox(
                height: 200,
                child: Center(
                  child: Column(
                    children: [
                      ...
                      Text('Here\'s nothing yet. Please add a transaction.', style: AppTextStyles.greyRegular),
                      const SizedBox(
                        height: 20,
                      ),
                      Icon(Icons.hourglass_empty_sharp, size: 30, color: AppColors.silver)
                    ],
                  ),
                ),
              )
                            ...
                            return GestureDetector(
                              ...
                              onLongPress: () {
                                if (dayTransactionsData[index].description.isNotEmpty) {
                                  _showDeleteOption(context, dayTransactionsData[index].description,
                                      dayTransactionsData[index].expenseId);
                                } else {
                                  _showDeleteOption(context, '${transactions.title} ${LocaleKeys.transaction}',
                                      dayTransactionsData[index].expenseId);
                                }
                              },
                              child: ListTile(
                                ...
                                subtitle:
                                    Text(dayTransactionsData[index].description, style: AppTextStyles.greyCategory),
                                trailing: Text(
                                  dayTransactionsData[index].amount.toString(),
                                  style: dayTransactionsData[index].amount < 0
