...
class TransactionHistoryWidget extends StatefulWidget {
  const TransactionHistoryWidget({Key? key, required this.page})
      : super(key: key);
  final String page;
            ...
            : Flexible(
                child: ListView.separated(
                  ...
                  itemBuilder: (
                    ...
                    final dayTransactionsData =
                        transactionsMap.elementAt(index).value;
                    return Container(
                        ...
                        children: [
                          ...
                          ...List.generate(dayTransactionsData.length, (index) {
                            final transactions = allCategories.firstWhere(
                              (element) =>
                                  element.categoryId ==
                                  dayTransactionsData[index].categoryId,
                            );
                            return GestureDetector(
                              onTapDown: (details) => _getTapPosition(details),
                              onLongPress: () {
                                if (dayTransactionsData[index]
                                    .description
                                    .isNotEmpty) {
                                  _showDeleteOption(
                                      context,
                              ...
                              child: ListTile(
                                leading: IconView(icon: transactions.icon.pathToIcon, color: transactions.icon.color,),
  ...
  void _showDeleteOption(BuildContext context, String title, int id) async {
    ...
    await showMenu(
      ...
      if (value != null) {
        context
            .read<DatabaseBloc>()
            .add(DeleteTransactionEvent(transactionId: id));
      }
    });
  }
}
