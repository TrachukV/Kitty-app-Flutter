...
class DateBloc extends Bloc<DateEvent, DateState> {
  DateBloc() : super(DateState(DateTime.now())) {
    on<IncMonthEvent>((event, emit) {
      emit(state.copyWith(dateTime: DateTime(state.dateTime.year, state.dateTime.month + 1)));
    });
    ...
    on<IncYearEvent>((event, emit) {
      emit(state.copyWith(dateTime: DateTime(state.dateTime.year - 1, state.dateTime.month)));
    });
